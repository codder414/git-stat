...
export class Cli {
    ...
    // private _applyValueTransformers() {
    //     try {
    //         for (let index in this._commands) {
    //             const command = this._commands[index];
    //             command.transformValue();
    //         }
    //     }
    //     catch (err) {
    //         throw new Error(`Cannot transform value: ${err.message}`);
    //     }
    // }
    ...
    public getValues() {
        return Object.assign({}, this._commands.reduce((acc, command) => {
