...
export async function main() {
  ...
  try {
    ...
    if (hash) {
      ...
      let gitLogResult = execa(
        "git",
        ["--no-pager", "log", "--decorate=short", '--pretty=format:"%H"'],
        { stdout: "pipe", cwd: projectPath }
      );
      gitShowResult = await execa(
        "xargs",
        [
    ...
    if (isPlain) {
      log(
        spliitedbyCommits.forEach((commit) =>
          log(
            `${commit.author}\t${commit.name}\t${commit.files.length}\t${commit.hash}\t${commit.date}`
          )
        )
      );
    } else if (isJson) {
